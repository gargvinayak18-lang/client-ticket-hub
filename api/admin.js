  ...
  try {
    ...
    switch (action) {
      case 'listStaff': {
        ...
        if (!roles || roles.length === 0) return res.json([]);
        ...
        return res.json((profiles || []).map(p => ({
          ...p, role: roles.find(r => r.user_id === p.id)?.role || 'jr_dev'
        })));
      }
      ...
      case 'createStaff': {
        ...
        const { error: roleErr } = await supabaseAdmin
          .from('user_roles').update({ role: data.role }).eq('user_id', created.user.id);
      ...
      case 'createAdmin': {
        ...
        if (error) throw new Error(error.message);
        const { error: roleErr } = await supabaseAdmin
          .from('user_roles').update({ role: 'admin' }).eq('user_id', created.user.id);
        if (roleErr) throw new Error(roleErr.message);
        return res.json({ id: created.user.id });
      }
      ...
      case 'updateRole': {
        ...
        const { error } = await supabaseAdmin
          .from('user_roles').update({ role: data.role }).eq('user_id', data.userId);
        if (error) throw new Error(error.message);
        return res.json({ success: true });
      }
      ...
      case 'addWebsite': {
        const { error } = await supabaseAdmin.from('websites').insert({
          client_id: data.client_id, name: data.name, url: data.url,
        });
        if (error) throw new Error(error.message);
      ...
      case 'listWebsites': {
        const { data: ws, error } = await supabaseAdmin
          ...
          .eq('client_id', data.client_id).order('created_at', { ascending: false });
        if (error) throw new Error(error.message);
        return res.json(ws || []);
      }
    ...
    return res.status(err.message === 'Unauthorized' ? 401 : err.message?.includes('Forbidden') ? 403 : 500)
